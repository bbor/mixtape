...
/*
  ...
  Options that you can specify in overall config, or per-record, or per-type:
              ...
              - layout: the name of a template, a template string, or a function that returns a template string.
              - toc
                - add_to_root
                - autoexpand
              - typeahead
...
mixtape.run = function(options, db, cb)
{
  ...
  function heading (title, anchor, level) {
    if (!title) return '';
    var html = `
<div class="heading">
<h` + level + `>
  ...
  function set_up_environment() {
    ...
    mrenderer.link = function(href, title, text) {
      ...
      return mrenderer.link_old(href, title, text);
    }
  ...
  function build_html_files() {
    ...
    function build_html_file(record) {
      ...
      function print_record(record, heading_level)
      {
        ...
        {
          record_context.content_title_html = marked(record.content_title);
        }

        if (!!record.content_summary)
        {
          record_context.content_summary_html = marked(record.content_summary);
        }

        // element content
        if (!!record.content)
        {
          record_context.content_html = marked(record.content);
        }
        ...
        // then add children
        html += print_children(record, heading_level);
