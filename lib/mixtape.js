...
mixtape.run = function(types, options, db, cb)
{
  ...
  function build_html_files() {
    ...
    function build_html_file(record) {
      var output_filename = path.join(config.output, record.target);
    ...
    // only print content if this is the right page where we belong
    function print_data_children(record, record_context, demote_headings_level) {
      ...
      if (children.length > 0)
      {
        children.forEach( function(child_uid) {
          ...
          record_context['content'] += '<section><p>content for ' + child.target + '</p>';
          if (types[child_type] && types[child_type].page === false) { record_context['content'] += '</section>'; return; }
          ...
          record_context['content'] += '</section>';
        })
      }
    }
  };
  ...
  function build_toc() {
    ...
    function print_toc_data_file(uid, child_uids)
    {
      ...
      child_uids.forEach(function(child_uid) {
        ...
        if (!record) { return; }
        var child_config = Object.assign({}, config, types[record.type], record);
        var data = {};
        data.id = record.uid;
        data.target = record.target;
        data.text = record.display_name || record.name;
        data.icon = (child_config.icon) ? child_config.icon : false;
