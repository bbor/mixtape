...
mixtape.context = {};
mixtape.context['code-languages'] = {};
...
mixtape.run = function(types, options, db, cb)
{
  ...
  function set_up_environment() {
    ...
    // and records the language tag so we can load the right prettyprint extension.
    mrenderer.code = function(code, lang, escaped) {
      if (lang)
      {
        lang = lang.match(/\w+/i)[0]; // take only plain text characters, skip any curly brackets and dots like {.css}
      }

      if (lang == 'javascript') lang = 'js'; // common mistake, so let's be nice.

      if (!lang || lang == '' || lang == 'nohighlight' || lang == 'c' || lang == 'cpp') {
        return '<pre><code>'
          + (escaped ? code : escape(code, true))
          + '\n</code></pre>';
      }

      mixtape.context['code-languages'][lang] = true;
      ...
      if (children.length > 0)
      {
        children.forEach( function(child_uid) {
          ...
          if (child.display_name || child.name)
            record_context['content'] += '<h' + max_h_level + '>' + (child.display_name || child.name) + '</h' + max_h_level + '>';
          record_context['content'] += '<p> TARGET: ' + child.target + '</p>'
