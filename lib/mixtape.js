...
mixtape.run = function(types, options, db, cb)
{
  ...
  function build_html_files() {
    ...
    // only pront content if this is the right page where we belong
  ...
  function build_typeahead() {
    var datapath = path.join(options.output, 'livesearch');
  ...
  function build_toc() {
    ...
    function print_toc_data_file(uid, child_uids)
    {
      ...
      child_uids.forEach(function(child_uid) {
        ...
        if (!record) { return; }
        var child_config = Object.assign({}, options, types[record.type], record);
        var data = {};
        data.id = record.uid;
