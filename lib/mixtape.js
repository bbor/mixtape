  ...
  function build_html_files() {
    ...
    function build_html_file(record) {
      ...
      function print_record(record, top_heading_level)
      {
        ...
        {
          html += heading(record.content_title, record.uid, top_heading_level);
        }
        ...
        {
          html += summary(record.content_summary);
        }
  ...
  function build_toc() {
    ...
    function print_toc_data_file(record, children)
    {
      ...
      child_uids.forEach(function(child_uid) {
        var child = db[child_uid];
        if (!child) { return; }
        var child_config = Object.assign({}, config, types[child.type], child);
        var child_data = {};
        child_data.id = 'toc_' + child.uid;
        child_data.target = child.target;
