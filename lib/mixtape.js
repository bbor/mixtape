...
<h` + level + ` id="` + anchor + `">`;
  ...
  function heading_close (anchor, level) {
    // TODO: also support icon? Or put that in the header itself?
    var html = `</h` + level + `>
</div>`;
    return new handlebars.SafeString(html);
  }
...
<div class="summary level` + level + `">`
  ...
  function print_children(heading_level) {
    var html = "";
    var child_uids = this.children;
    if (child_uids.length == 0) return '';

    // we need to resolve the children's uids into their records
    var children = [];
  ...
  function build_toc() {
    ...
    function print_toc_data_file(record)
    {
      function get_toc_data_for_children(record) {
        ...
        children.forEach(function(child) {
          if (!child['mixtape-page'] || !child['mixtape-target']) { return; }
          var child_config = merge_options.call({concatArrays: true}, {}, config, config.types[child.type], child);
          var child_data = {};
