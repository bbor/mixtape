...
<p>` + summary + `</p>
  ...
  function print_record(record, heading_level)
  {
    ...
    {
      record_context.content_title_html = markdownIt.renderInline(record.content_title);
    }

    if (!!record.content_summary)
    {
      record_context.content_summary_html = markdownIt.renderInline(record.content_summary);
    }

    // element content
    if (!!record.content)
    {
      record_context.content_html = markdownIt.render(record.content);
    }
